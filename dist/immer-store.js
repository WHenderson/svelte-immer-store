...
exports.immerStore = exports.noop = void 0;
const immer_1 = require("immer");
const path_1 = require("./proxy/path");
const tracker_1 = require("./proxy/tracker");
const noop = () => { };
exports.noop = noop;
var ImmerStoreState;
...
function immerStore(value, start = exports.noop, record) {
    // store subscriptions
    const subscriptions = [];
    // called when subscribers reaches zero
    let stop;
    // current state
    let state = immer_1.default(value, () => { });
    ...
    function findPath(propertyOrPathOrSelector, relative) {
        ...
        return propertyOrPathOrSelector(path_1.createPathProxy())[path_1.symPath];
    }
    ...
    // update state
    function update(fn) {
        const result = immer_1.default(state, draft => {
    ...
    // subscribe to store
    function subscribe(run, invalidate = exports.noop) {
        const subscription = {
            subscriber: run,
            invalidator: invalidate
        };
    ...
    function select(propertyOrPathOrSelector, relative) {
        ...
        function subSet(new_value) {
            update((old_value) => {
                const { value, parent, property } = selector(tracker_1.createTrackerProxy(old_value))[tracker_1.symTrackerDetails];
                if (parent) {
        ...
        // subscribe
        function subSubscribe(run, invalidate = exports.noop) {
